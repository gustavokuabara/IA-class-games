...
public class GerenciadorGrade : MonoBehaviour
{
    ...
    public bool pesoAtivos = true;

    

    //popup quando n�o � possivel resolver o labirinto
    ...
    void GerarGrade()
    {
        ...
        for (int x = 0; x < largura; x++)
        {
            for (int y = 0; y < altura; y++)
            {
                ...
                if (pesoAtivos && node.ehAndavel && tilePrefab != tileInicio && tilePrefab != tileDestino)
                {
                    float peso = Random.Range(1, 11);
                    node.peso = peso;
                    node.GetComponent<SpriteRenderer>().color = gradient.Evaluate(peso * 25 / 255f);
                }
    ...
    public void TogglePeso(){
        pesoAtivos = !pesoAtivos;
    }
